...
def add_sentiment_scores(df: pd.DataFrame) -> pd.DataFrame:
    """
    Calculates sentiment scores for page titles using the faster VADER library.
    VADER is specifically tuned for social media and micro-blogging, and it's fast.
    """
    ...
    def get_vader_compound_score(title):
        # VADER works best on raw text
        return analyzer.polarity_scores(str(title))['compound']

    print("Running sentiment analysis with VADER...")

    # We only need to analyze titles for specific SERP features
    ...
    if mask.sum() > 0:
        # Apply the function only to the relevant subset of the DataFrame
        # This is much faster than TextBlob's apply
        sentiment_scores = df.loc[mask, 'PageTitle'].apply(
...
def prepare_chart_data(paa_df: pd.DataFrame, sv_df: pd.DataFrame) -> pd.DataFrame:
    ...
    This function remains largely the same as it uses efficient pandas operations.
    """
    ...
    # A score from -0.05 to 0.05 is typically considered neutral.
    conditions = [
        sentiment_recent['sentiment'] > 0.05,
        sentiment_recent['sentiment'] < -0.05
    ]
    choices = ['Positive', 'Negative']
    sentiment_recent['Label'] = np.select(
    ...
    sentiment_aggregated = sentiment_recent.groupby(['SearchTerm', 'Market']).agg(
        results_analysed=('Label', lambda x: (
            x.isin(['Positive', 'Negative'])).sum()),
    ...
    sv_df['GrowthorDecline'] = sv_df.groupby(['SearchTerm', 'Market'])[
        'SearchVolume'].pct_change() * 100
    ...
    final_df['Positive_Percentage'] = (
        final_df['positive_count'] / denominator * 100).fillna(0)
    final_df['Negative_Percentage'] = (
        final_df['negative_count'] / denominator * 100).fillna(0)
    final_df['Sentiment'] = ((final_df['positive_count'] -
                             final_df['negative_count']) / denominator * 100).fillna(0)
    ...
    numeric_cols = ['search_volume_12m_avg', 'growth_pct',
                    'Positive_Percentage', 'Negative_Percentage', 'sentiment_score']
